...
To achieve 30 million logs per second, string information is pre-processed at compile-time. At run-time, only arguments are stored. <br>
Furthermore, compact storage is obtained by writing duplicate string only once, coupled with generalized delta-encoding.
...
Highlights:
 ...
 - Compilation time cost per unit is around 0.7 s
   ...
   - Compilation time cost per log is ~1.5 µs due to the compile-time work to speed-up runtime
