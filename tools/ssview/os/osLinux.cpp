...
static struct {
    ...
    GLXContext xRenderContext = nullptr;
    Window     windowHandle;
    ...
    int        dpiHeight     = -1;
    // Character inputs
    XIM xInputMethod  = nullptr;  // Input method linked to the X display
    XIC xInputContext = nullptr;  // Input context used to gete unicode input in our window
    // Others
    bsString appPath;
    bool     isDirectOverride = false;
    // Application
    os::Handler* osHandler = nullptr;
} gGlob;
...
static struct {
    ...
    Atom              aMyProperty;
    Atom              aTargets;
    os::ClipboardType ownedType = os::ClipboardType::NONE;
    bsString          ownedData;
...
createWindow(const char* windowTitle, const char* configName, float ratioLeft, float ratioTop, float ratioRight, float ratioBottom,
             bool overrideWindowManager)
{
    asserted(ratioLeft >= 0. && ratioLeft <= 1.);
    asserted(ratioTop >= 0. && ratioTop <= 1.);
    asserted(ratioRight >= 0. && ratioRight <= 1.);
    asserted(ratioBottom >= 0. && ratioBottom <= 1.);
    asserted(ratioLeft < ratioRight);
    asserted(ratioTop < ratioBottom);

    gGlob.isDirectOverride = overrideWindowManager;
    gGlob.xDisplay         = XOpenDisplay(nullptr);
    asserted(gGlob.xDisplay, "Unable to connect to X server");
    ...
    for (int i = 0; i < fbconfigQty; ++i) {
        ...
        if (pict_format &&
            pict_format->direct.alphaMask == 0) {  // Note: choose pict_format->direct.alphaMask> 0 for transparent background...
    ...
    XFree(fbconfigs);
    asserted(fbconfig, "No matching GLX frame buffer config found");

    // Describe FB config (display only)
    {
        int doublebuffer = 0;
        int red_bits = 0, green_bits = 0, blue_bits = 0, alpha_bits = 0, depth_bits = 0;

        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_DOUBLEBUFFER, &doublebuffer);
        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_RED_SIZE, &red_bits);
        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_GREEN_SIZE, &green_bits);
        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_BLUE_SIZE, &blue_bits);
        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_ALPHA_SIZE, &alpha_bits);
        glXGetFBConfigAttrib(gGlob.xDisplay, fbconfig, GLX_DEPTH_SIZE, &depth_bits);

        /*
        fprintf(stderr, "FBConfig selected:\nDoublebuf: %s\nRGBA %d/%d/%d/%d, Depth Bits: %d\n",
                doublebuffer ? "Yes" : "No", red_bits, green_bits, blue_bits, alpha_bits, depth_bits);
        */
    }
...
#define COMPUTE_DPI(pix, mm)                                                                                                              \
    ...
    gGlob.windowHandle    = XCreateWindow(gGlob.xDisplay, Xroot, x, y, gGlob.wWidth, gGlob.wHeight, 0, visual->depth, InputOutput,
                                          visual->visual, attr_mask, &attr);
    gGlob.glXWindowHandle = gGlob.windowHandle;

    if (!gGlob.windowHandle) { fatalError("Couldn't create the window\n"); }

    // Monitors
    // =========
    int dummy1 = 0, dummy2 = 0;
    int major = 0, minor = 0;
    if (!XRRQueryExtension(gGlob.xDisplay, &dummy1, &dummy2) || !XRRQueryVersion(gGlob.xDisplay, &major, &minor)) {
        asserted(false, "XRRQueryExtension failed\n");
    }
    asserted(major > 1 || minor >= 5, "Too old Xrandr version");
    bool            onlyActive = false;  // if set it returns only active monitors (non-0x0 monitors)
    int             monitorCnt = 0;
    XRRMonitorInfo* xMonitors  = XRRGetMonitors(gGlob.xDisplay, gGlob.windowHandle, onlyActive, &monitorCnt);
    asserted(xMonitors, "XRRGetMonitors failed to get the monitor description");
    for (int monitorIdx = 0; monitorIdx < monitorCnt; monitorIdx++) {
        const XRRMonitorInfo& m = xMonitors[monitorIdx];
        printf("Monitor %-10s (+%d+%d %dx%d%s)\n", XGetAtomName(gGlob.xDisplay, m.name), m.x, m.y, m.width, m.height,
               m.primary ? " primary" : "");
    }
    ...
    gGlob.xInputMethod = XOpenIM(gGlob.xDisplay, nullptr, nullptr, nullptr);
    ...
    asserted(status, "OpenGL not supported by X server\n");

    typedef GLXContext (*glXCreateContextAttribsARBProc)(Display*, GLXFBConfig, GLXContext, Bool, const int*);
    glXCreateContextAttribsARBProc glXCreateContextAttribsARB =
        (glXCreateContextAttribsARBProc)glXGetProcAddressARB((const GLubyte*)"glXCreateContextAttribsARB");
    asserted(glXCreateContextAttribsARB, "No support for OpenGL3 or GLX1.4\n");

#define GLX_CONTEXT_MAJOR_VERSION_ARB 0x2091
#define GLX_CONTEXT_MINOR_VERSION_ARB 0x2092
    int context_attribs[]                     = {GLX_CONTEXT_MAJOR_VERSION_ARB, 3, GLX_CONTEXT_MINOR_VERSION_ARB, 3, None};
    int (*oldHandler)(Display*, XErrorEvent*) = XSetErrorHandler(&ctxErrorHandler);  // Temporarily installed to get a
                                                                                     // meaningful message in case of error
    gGlob.xRenderContext = glXCreateContextAttribsARB(gGlob.xDisplay, fbconfig, nullptr, True, context_attribs);
    XSync(gGlob.xDisplay, False);
    XSetErrorHandler(oldHandler);
    asserted(gGlob.xRenderContext, "Failed to create a GL context\n");
    status = glXMakeContextCurrent(gGlob.xDisplay, gGlob.glXWindowHandle, gGlob.glXWindowHandle, gGlob.xRenderContext);
    asserted(status, "glXMakeContextCurrent failed for window\n");
    ...
    gClip.aMyApp      = XInternAtom(gGlob.xDisplay, "GRAPH_VIEWER", False);
    ...
    GC     graphicsContext = XCreateGC(gGlob.xDisplay, cursorPixmap, 0, nullptr);
    ...
    if (!os::directoryExists(gGlob.appPath.toChar())) {
        if (mkdir(gGlob.appPath.toChar(), S_IRWXU) < 0) { printf("Error: Unable to create the folder %s\n", gGlob.appPath.toChar()); }
    }
}
...
destroyWindow()
{
    glXMakeCurrent(gGlob.xDisplay, 0, nullptr);
...
showWindow()
{
    ...
    XSync(gGlob.xDisplay, False);
    if (gGlob.isDirectOverride) { XSetInputFocus(gGlob.xDisplay, gGlob.windowHandle, RevertToParent, CurrentTime); }
}

void
setIcon(int width, int height, const uint8_t* pixels)
{
    ...
    uint32_t defDepth  = DefaultDepth(gGlob.xDisplay, screen);
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < pitch; ++i) {
            for (int k = 0; k < 8; ++k) {
                ...
                uint8_t alpha = (pixels[(i * 8 + k + j * width) * 4 + 3] > 0) ? 1 : 0;
    ...
    *ptr++              = height;
    ...
    XChangeProperty(gGlob.xDisplay, gGlob.windowHandle, netWmIcon, XA_CARDINAL, 32, PropModeReplace,
                    reinterpret_cast<const uint8_t*>(&icccmIconPixels[0]), 2 + width * height);
    XFlush(gGlob.xDisplay);
}
...
{
    if (state == os::isMouseVisible()) return;
...
{
    return fopen(path.toChar(),
                 mode);  // Utf-8 does not change the fopen prototype on Linux!
}
...
loadFileContent(const bsString& path, bsVec<uint8_t>& buffer, int maxSize)
{
    // Get file size
    buffer.clear();
    size_t fileSize = os::getSize(path);
    if (fileSize == 0) return false;
    if (maxSize > 0 && fileSize > (size_t)maxSize) fileSize = maxSize;
    // Open it
    FILE* fh = os::fileOpen(path, "rb");
    if (!fh) return false;
    // Read it in buffer
...
makeDir(const bsString& path)
{
    ...
    if (tmp.empty()) return DirStatusCode::OK;
    for (uint8_t* p = &tmp[1]; *p; ++p) {
        if (*p == '/') {
            *p         = 0;
            int status = mkdir(tmp.toChar(), S_IRWXU);
            if (status != 0 && errno != EEXIST) {
                if (errno == EACCES) return DirStatusCode::PERMISSION_DENIED;
                return DirStatusCode::FAILURE;
            }
            *p = '/';
        }
    }
    int status = mkdir(tmp.toChar(), S_IRWXU);
    if (status == 0)
        return DirStatusCode::OK;
    else if (errno == EACCES)
        return DirStatusCode::PERMISSION_DENIED;
    else if (errno == EEXIST)
        return DirStatusCode::ALREADY_EXISTS;
    return DirStatusCode::FAILURE;
}
...
getDirContent(const bsString& path, bsVec<DirEntry>& entries)
{
    ...
    if (!dir) {
        if (errno == EACCES)
            ...
            return DirStatusCode::NOT_A_DIRECTORY;
        else if (errno == ENOENT)
            return DirStatusCode::DOES_NOT_EXIST;
        return DirStatusCode::FAILURE;
    }
    ...
    while ((fe = readdir(dir))) {
        if (fe->d_type == DT_REG || (fe->d_type == DT_DIR && strcmp(fe->d_name, ".") != 0 && strcmp(fe->d_name, "..") != 0)) {
            entries.push_back(DirEntry{fe->d_name, fe->d_type == DT_DIR});
        }
    }
...
getSize(const bsString& path)
{
    ...
    while ((fe = readdir(dir))) {
        if (fe->d_type == DT_REG || (fe->d_type == DT_DIR && strcmp(fe->d_name, ".") != 0 && strcmp(fe->d_name, "..") != 0)) {
            dirSize += os::getSize((path + "/") + bsString(fe->d_name));
        }
    }
    closedir(dir);
    return dirSize;
...
getDate()
{
    ...
    return Date{1900 + t->tm_year, 1 + t->tm_mon, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec};
}
...
{
    struct stat s;
    if (stat(path.toChar(), &s) == -1) return Date();  // If error, all fields are zero
...
DirStatusCode
removeDir(const bsString& path, bool /*onlyIfEmpty*/)
{
    // Open directory
    DIR* dir = opendir(path.toChar());
    if (!dir) {
    ...
    // Remove it, now that it is empty
...
copyFile(const bsString& srcPath, const bsString& dstPath)
{
    ...
    if (!(fDst = fileOpen(dstPath, "wb"))) goto end;
    isSuccess = true;
...
reqFromClipboard(ClipboardType /*reqType*/)
{
    ...
    while (!gClip.isReqReceived && bsGetClockUs() < startUs + CLIPBOARD_REQ_TIMEOUT_US) {
        bsSleep(1000);
        os::processInputs(gGlob.osHandler);
    }
...
answerReqClipboard()
{
    Atom           da = 0, incr = 0, type = 0;
    int            di   = 0;
    uint64_t       size = 0, dul = 0;
    unsigned char* prop_ret = nullptr;
    // Dummy call to get type and size
    XGetWindowProperty(gGlob.xDisplay, gGlob.windowHandle, gClip.aMyProperty, 0, 0, False, AnyPropertyType, &type, &di, &dul, &size,
                       &prop_ret);
    XFree(prop_ret);
    ...
    XGetWindowProperty(gGlob.xDisplay, gGlob.windowHandle, gClip.aMyProperty, 0, size, False, AnyPropertyType, &da, &di, &dul, &dul,
                       &prop_ret);
    if (!prop_ret) return;
    uint8_t* begin     = prop_ret;
    uint8_t* end       = prop_ret + size;
...
targetsPushClipboard(XSelectionRequestEvent* sev)
{
    ...
    XChangeProperty(gGlob.xDisplay, sev->requestor, sev->property, XA_ATOM, 32, PropModeReplace, (uint8_t*)&targets[0],
...
{
    XChangeProperty(gGlob.xDisplay, sev->requestor, sev->property, gClip.aUtf8, 8, PropModeReplace, &gClip.ownedData[0],
                    gClip.ownedData.size());
    XSelectionEvent ssev;
    ssev.type      = SelectionNotify;
    ssev.requestor = sev->requestor;
    ssev.selection = sev->selection;
        ...
        */
        ...
        switch (event.type) {
            ...
            case SelectionRequest: {
                ...
                if (sev->target == gClip.aTargets)
                    targetsPushClipboard(sev);
                else if (sev->target != gClip.aUtf8 || sev->property == None || gClip.ownedData.empty())
                    denyPushClipboard(sev);
                else
                    answerPushClipboard(sev);
            } break;
            ...
            case KeyPress:
                ...
                if (gGlob.xInputContext) {
                    ...
                    if (length > 0) {
                        ...
                        if (codepoint != 0 && bsIsUnicodeDisplayable(codepoint)) gGlob.osHandler->eventChar((uint16_t)codepoint);
                    }
                } else
#endif
            ...
            case KeyRelease:
                ...
                break;
            case FocusIn:
                if (gGlob.xInputContext) { XSetICFocus(gGlob.xInputContext); }
                break;
            case FocusOut:
                if (gGlob.xInputContext) { XUnsetICFocus(gGlob.xInputContext); }
                if (gGlob.osHandler->isVisible() && event.xfocus.mode == NotifyNormal) { gGlob.osHandler->notifyFocusOut(); }
                break;
            case EnterNotify:
                kms = {(bool)(event.xcrossing.state & ShiftMask), (bool)(event.xcrossing.state & ControlMask),
                       (bool)(event.xcrossing.state & Mod1Mask), (bool)(event.xcrossing.state & Mod4Mask)};
                gGlob.osHandler->notifyEnter(kms);  // KMS and mouse resynchronization are required
                gGlob.osHandler->eventMouseMotion(event.xcrossing.x, event.xcrossing.y);
                break;
            case LeaveNotify:
                kms = {(bool)(event.xcrossing.state & ShiftMask), (bool)(event.xcrossing.state & ControlMask),
                       (bool)(event.xcrossing.state & Mod1Mask), (bool)(event.xcrossing.state & Mod4Mask)};
                ...
                break;
            case MapNotify:
                if (!gGlob.osHandler->isVisible()) { gGlob.osHandler->notifyMapped(); }
                break;
            case UnmapNotify:
                ...
                break;
            case ButtonPress:
                kms = {(bool)(event.xbutton.state & ShiftMask), (bool)(event.xbutton.state & ControlMask),
                       (bool)(event.xbutton.state & Mod1Mask), (bool)(event.xbutton.state & Mod4Mask)};
                if (event.xbutton.button == 4)
                    gGlob.osHandler->eventWheelScrolled(event.xbutton.x, event.xbutton.y, -1, kms);
                else if (event.xbutton.button == 5)
                    gGlob.osHandler->eventWheelScrolled(event.xbutton.x, event.xbutton.y, +1, kms);
                else
                    gGlob.osHandler->eventButtonPressed(event.xbutton.button, event.xbutton.x, event.xbutton.y, kms);
                break;
            case ButtonRelease:
                kms = {(bool)(event.xbutton.state & ShiftMask), (bool)(event.xbutton.state & ControlMask),
                       (bool)(event.xbutton.state & Mod1Mask), (bool)(event.xbutton.state & Mod4Mask)};
                if (event.xbutton.button < 4)
                    gGlob.osHandler->eventButtonReleased(event.xbutton.button, event.xbutton.x, event.xbutton.y, kms);
                break;
            case MotionNotify:
...
{
    return osBootstrapImpl(argc, argv);
}
